...
x-i18n:
  ...
  generated_at: '2026-02-27T20:16:53.011Z'
---

# Synthetic

Synthetic cung cấp các endpoint tương thích với Anthropic. OpenClaw đăng ký nó dưới dạng nhà cung cấp `synthetic` và sử dụng API Anthropic Messages.
## Bắt đầu nhanh
...
Tất cả các mô hình dưới đây sử dụng chi phí `0` (đầu vào/đầu ra/bộ nhớ đệm).

| ID mô hình                                               | Cửa sổ ngữ cảnh | Số token tối đa | Lập luận | Đầu vào      |
| ------------------------------------------------------ | -------------- | ---------- | --------- | ------------ |
...
- Nếu bạn bật danh sách cho phép mô hình (`agents.defaults.models`), hãy thêm mọi mô hình bạn
  dự định sử dụng.
- Xem [Nhà cung cấp mô hình](/concepts/model-providers) để biết các quy tắc nhà cung cấp.